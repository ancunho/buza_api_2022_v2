...
import com.buza.server.dto.AccountUser;
...
//        return new Date(30 * 24 * 60);
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        AccountUser accountUser = new AccountUser(userDetails.getUsername(), userDetails.getPassword(), userDetails.getAuthorities());

        final String username = getUsernameFromToken(token);
        final Date created = getCreatedDateFromToken(token);
        return (username.equals(accountUser.getUsername()) && !isTokenExpired(token));
    }
